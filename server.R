...
Sci Rep. 2023 Oct 7;13(1):16922. doi: 10.1038/s41598-023-44109-3. "})
  output$text_info2g<-renderText({"Latest update: 08.08.2025"})
  ...
  observeEvent(input$do_in,{
    if(input$ownData=="Load demo data"){
      shinyjs::html("text", paste0("<br>Demo data is loaded.<br><br>"), add = FALSE)
      
      input2<-read.table("www/Demo_data.txt",
                         header=T,quote = "",comment.char = "",sep="\t",stringsAsFactors = F) 
      

      
      shinyjs::html("text", paste0("Demo data successfully loaded","<br>"), add = TRUE) 
    }else{
    ...
    if(input$ownData=="Load demo data"){
      output$columnUI5<-renderUI({
        selectInput('column5',label = HTML("...first antimicrobial agent:"),choices = names(input2),selected="Amoxicillin")
      })
    }else{
      output$columnUI5<-renderUI({
    ...
    output$bakteriumUI0b<-renderUI({
      material_helper<-unique(input2$ORD_MATERIAL[format(helper1bakterien,"%Y")%in%input$analysis_type1a])
